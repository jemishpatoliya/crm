...
export const ManagerLeadsPage = () => {
  ...
  const handleAddLead = async () => {
    ...
    await mockApi.post("/leads", { ...newLead, status: "NEW", tenantId: "t_soundarya" });
    await loadLeads();
    setIsAddOpen(false);
    ...
    <PageWrapper title="Lead Management" description="Manage and assign leads to your team." sidebarCollapsed={sidebarCollapsed}
      actions={
        <div className="flex gap-2">
          ...
          <Button variant="outline" size="sm" onClick={() => fileInputRef.current?.click()}><Upload className="w-4 h-4 mr-2" />Import CSV</Button>
          <Dialog open={isAddOpen} onOpenChange={setIsAddOpen}>
            <DialogTrigger asChild><Button size="sm"><Plus className="w-4 h-4 mr-2" />Add Lead</Button></DialogTrigger>
    ...
    >
          ...
          <div className="bg-card rounded-lg border border-border overflow-hidden">
            ...
            <div className="flex items-center justify-between px-4 py-3 border-t border-border">
              ...
              <div className="flex gap-2">
                ...
                <Button variant="outline" size="sm" disabled={currentPage === totalPages || totalPages === 0} onClick={() => setCurrentPage((p) => Math.min(totalPages, p + 1))}>Next</Button>
              </div>
            </div>
          </div>
        )}
      ...
      <ActionBottomBar selectedCount={selectedIds.size} onClose={() => setSelectedIds(new Set())}>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent className="bg-popover">{agents.map((a) => (<DropdownMenuItem key={a.id} onClick={() => handleBulkAssign(a.id)}>{a.name}</DropdownMenuItem>))}</DropdownMenuContent>
        </DropdownMenu>
